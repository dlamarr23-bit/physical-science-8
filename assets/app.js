...
(function(){
  ...
  function loadVisuals(){
    ...
    script.src='./assets/visuals-rich.js?v=approved-posters-2';
    script.defer=true;
  ...
  function initReveal(){document.querySelectorAll("[data-reveal]").forEach(function(btn){btn.addEventListener("click",function(){var target=document.getElementById(btn.getAttribute("data-reveal"));if(target){var hidden=target.hasAttribute("hidden");if(hidden)target.removeAttribute("hidden");else target.setAttribute("hidden","");btn.setAttribute("aria-expanded",hidden?"true":"false");}});});}
  document.addEventListener("DOMContentLoaded",function(){injectIcons();loadVisuals();initDrawer();initVocab();initQuizzes();initGlossaryFilter();initGlossaryTerms();initReveal();var body=document.body;markVisited(body&&body.getAttribute("data-chapter"));paintNavChecks();});
})();
